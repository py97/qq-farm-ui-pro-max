...
if [ -d "$SYNC_DIR" ]; then
    echo "🗑️  清理旧的同步文件夹..."
    find "$SYNC_DIR" -maxdepth 1 -mindepth 1 ! -name '.git' -exec rm -rf {} +
fi
...
# web 构建所需配置文件
for f in tsconfig.json tsconfig.app.json tsconfig.node.json vite.config.ts eslint.config.js uno.config.ts; do
    if [ -f "web/$f" ]; then
        cp "web/$f" "$SYNC_DIR/web/"
    fi
done
if [ -d "web/public" ]; then
    cp -r web/public "$SYNC_DIR/web/"
fi
...
if [ -d "assets" ]; then
    cp -r assets/* "$SYNC_DIR/assets/"
fi
if [ -f "CHANGELOG.md" ]; then
    cp CHANGELOG.md "$SYNC_DIR/"
fi

# 复制 deploy 部署目录（完整）
echo "📄 复制部署目录..."
if [ -d "deploy" ]; then
    mkdir -p "$SYNC_DIR/deploy"
    cp -r deploy/* "$SYNC_DIR/deploy/" 2>/dev/null || true
fi
...
if [ -f "docker-compose.yml" ]; then
    ...
    cp docker/docker-compose.yml "$SYNC_DIR/"
fi
if [ -f "docker-compose.prod.yml" ]; then
    cp docker-compose.prod.yml "$SYNC_DIR/"
elif [ -f "docker/docker-compose.prod.yml" ]; then
    cp docker/docker-compose.prod.yml "$SYNC_DIR/"
fi
if [ -f "Dockerfile" ]; then
    cp Dockerfile "$SYNC_DIR/"
elif [ -f "core/Dockerfile" ]; then
    cp core/Dockerfile "$SYNC_DIR/core/"
fi
# 复制 Update.log（Dockerfile 构建需要）
mkdir -p "$SYNC_DIR/logs/development"
if [ -f "logs/development/Update.log" ]; then
    cp logs/development/Update.log "$SYNC_DIR/logs/development/"
fi
if [ -f "start.sh" ]; then
